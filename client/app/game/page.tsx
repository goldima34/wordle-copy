...
const Page = () => {
    ...
    return (
        <div className='container'>
            <div className="game-grid">
                {arr.map((row, indexRow) => 
                    row.map((col, indexCol) => 
                        <div className="cell" id={`indexRow-${indexRow}-indexCol-${indexCol}`} key={`${indexRow}-${indexCol}`}>
                ...
                {keyboard.map((row, rowIndex) => (
                    <div className="keyboard-row" key={rowIndex}>
                        {row.map((key, keyIndex) => (
                            <div 
                                className={`key ${key === 'backspace' ? 'backspace' : key === 'enter' ? 'enter' : ''}`} 
                                key={keyIndex} 
            ...
            {notification && (
                <div className={`notification ${showNotification ? 'show' : ''} ${hideNotification ? 'hide' : ''}`}>
                    {notification}
                </div>
            )}
            <div>{gameOver === "loose" && <ModalLoose word={word} attemps={currentRow}/>}</div>
            <div>{gameOver === "win" && <ModalWin word={word} attemps={currentRow}/>}</div>
